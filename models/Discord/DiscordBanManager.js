...
class DiscordBanManager {
    ...
    setup() {
      setInterval(() => {this.handleTempbans();}, 15000);
    }

    save() {
      fs.writeFile('./config/softbans.json', JSON.stringify(this.bans, null, 4), 'utf8', function(error){
    ...
    ban(guildMember, reason, time) {
      ...
      var ban = { userID: guildMember.user.id,
